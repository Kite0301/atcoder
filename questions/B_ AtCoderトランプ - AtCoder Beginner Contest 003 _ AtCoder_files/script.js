...
//      tr.append($('<td/>').addClass('standings-rank').html('<a href="/standings#page_' + ATCODER.standings.me.page + '">' + row.rank + '</a>'));
    ...
    if(row.contest_screen_name != null) {
      td_username.append(
        ...
        .html(' <i class="icon-search tooltip-label" rel="tooltip" data-title="' + row.user_name + 'の提出を確認" data-original-title=""></i>')
      );
    } else {
      td_username.append(
  ...
  var server_time = (new Date($('#server-current-time').html())).getTime() + time_offset;
  var contest_begin_time = (new Date($('#contest-start-time').html())).getTime() + time_offset;
  var contest_end_time   = (new Date($('#contest-end-time').html())).getTime() + time_offset;
